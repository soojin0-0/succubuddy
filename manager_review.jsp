...
<%
    ...
    try {
        ...
        if (hasKeyword) {
            countSql += " WHERE user_id LIKE ? OR review_text LIKE ?";
        }
        ...
        // 2) 회원 조회 (검색 적용 + 페이징)
        String sql = "SELECT review_id, user_id, product_id, review_text, review_score, review_ymd, review_pswd " +
                     "FROM review";
        if (hasKeyword) {
            sql += " WHERE user_id LIKE ? OR review_text LIKE ?";
        }
...
<script>
    function confirmDelete() {
        return confirm("이 리뷰를 정말로 지우시겠습니까?");
    }
</script>
</head>
<body>
...
<center>
<div class="title"><a href="manager_review.jsp">리뷰관리</a></div>
...
%>
    <tr>
        ...
        <td><%= rs.getInt("review_score") %></td>
        <td><%= rs.getString("review_ymd") %></td>
        <td><%= rs.getString("review_pswd") %></td>
		<td>
			<form method="post" action="manager_review_delete.jsp" onsubmit="return confirmDelete();">
