...
<%
    ...
    try {
        ...
        String countSql = hasKeyword ?
			"SELECT COUNT(*) FROM inquiry WHERE product_id IS NULL AND inquiry_subject LIKE ?" :
			"SELECT COUNT(*) FROM inquiry WHERE product_id IS NULL";

        pstmt = conInquiry.prepareStatement(countSql);
        if (hasKeyword) {
        ...
        // 목록 조회 SQL
        int start = (currentPage - 1) * inquiriesPerPage;
        String listSql = hasKeyword ?
			"SELECT inquiry_id, inquiry_ymd, inquiry_subject, status AS inquiry_status FROM inquiry WHERE product_id IS NULL AND inquiry_subject LIKE ? ORDER BY inquiry_ymd DESC LIMIT ?, ?" :
			"SELECT inquiry_id, inquiry_ymd, inquiry_subject, status AS inquiry_status FROM inquiry WHERE product_id IS NULL ORDER BY inquiry_ymd DESC LIMIT ?, ?";

        pstmt = conInquiry.prepareStatement(listSql);
        if (hasKeyword) {
            pstmt.setString(1, "%" + keyword + "%");
            pstmt.setInt(2, start);
            pstmt.setInt(3, inquiriesPerPage);
        } else {
            pstmt.setInt(1, start);
            pstmt.setInt(2, inquiriesPerPage);
        }

        rsInquiry = pstmt.executeQuery();
%>
...
<div class="tab-menu">
  ...
  <div class="tab active" onclick="location.href='sub5-inquiry.jsp'">문의</div>
  <div class="tab" onclick="location.href='sub5-notice.jsp'">공지사항</div>
</div>

<div class="search">
  <form method="get" action="sub5-inquiry.jsp">
	...
	%>
			<tr>
				<td colspan="3" style="text-align: center; font-size: 24px;">작성된 문의가 없습니다.</td>
			</tr>
	<%
			}

			while (rsInquiry.next()) {
				String date = rsInquiry.getString("inquiry_ymd").split(" ")[0]; // 시간 제거
				String subject = rsInquiry.getString("inquiry_subject");
				String status = rsInquiry.getString("inquiry_status"); // 기본값: 답변대기
	%>
			<tr>
				<td class="contents"><%= date %></td>
				<td class="contents">
				  <a href="sub5-inquiry-detail.jsp?inquiry_id=<%= rsInquiry.getInt("inquiry_id") %>" style="color: inherit; text-decoration: none;">
					<%= subject %>
				  </a>
				</td>
				<td class="contents">
					...
					%>
						<button style="background-color: #f9c74f; color: black; border: none; width: 120px; height: 40px; border-radius: 18px; font-weight: bold; cursor: default; font-family: 'GmarketSansTTFLight'; font-size: 20px;" disabled>
							답변대기
						</button>
					<%
						} else if ("답변완료".equals(status)) {
					%>
						<button style="background-color: #60af46; color: white; border: none; width: 120px; height: 40px; border-radius: 18px; font-weight: bold; cursor: default; font-family: 'GmarketSansTTFLight'; font-size: 20px;" disabled>
							답변완료
						</button>
					<%
						}
	...
	<%
			...
			e.printStackTrace();
			out.println("<tr><td colspan='3' style='color: red;'>문의 데이터를 불러오는 중 오류 발생: " + e.getMessage() + "</td></tr>");
		} finally {
			try {
	...
	<div class="write">
      <button class="write-btn">
	    <a href="sub5-inquiry-write.jsp" class="write-btn">글쓰기</a>
	  </button>
   </div>
...
%>
    <a href="sub5-inquiry.jsp?page=<%= currentPage - 1 %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>">&lt;</a>
<%
        }

        for (int i = 1; i <= totalPages; i++) {
%>
    <a href="sub5-inquiry.jsp?page=<%= i %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>" 
