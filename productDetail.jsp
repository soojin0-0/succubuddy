...
<%@ page import="java.util.*, java.util.Map, java.util.HashMap" %>
...
<%
    ...
    try {
        ...
        if (rs.next()) {
            ...
            try {
                productPrice = String.format("%,d", Integer.parseInt(productPrice));
        ...
        if (rs.next()) {
            averageRating = rs.getDouble("avg_score");
        ...
        String noob = "";
        int category = 0;
        String sqlNoob = "SELECT noob, category FROM succu_experience WHERE id = ?";
        pstmt = conn.prepareStatement(sqlNoob);
        pstmt.setString(1, product_id);
        rs = pstmt.executeQuery();
        if (rs.next()) {
            noob = rs.getString("noob");
        ...
        // 초보자용 상품 ID 목록
		List<String> beginnerProducts = Arrays.asList("s006", "m003", "s003", "s004", "s005", "s001", "m007", "m008", "m001");
		// 중상급자용 상품 ID 목록
		List<String> advancedProducts = Arrays.asList(
			"m002", "l001", "l006", "m004", "m005", "m006",
			"s002", "s007", "s008", "l007", "l002", "l004", "l005", "l003"
		);
		boolean isBeginner = beginnerProducts.contains(product_id);
		boolean isAdvanced = advancedProducts.contains(product_id);

		// 조건에 맞는 화분 필터링
		String potSql = "";
		String size = "";
			if (product_id.startsWith("s")) size = "소";
			else if (product_id.startsWith("m")) size = "중";
			else if (product_id.startsWith("l")) size = "대";
		if (isBeginner) {
			// 상품 코드 앞자리로 크기 결정
			potSql = "SELECT pot_id, pot_name, extra_price, image_name FROM pot WHERE level = '초보자' AND size = ?";
			PreparedStatement potPstmt = conn.prepareStatement(potSql);
			potPstmt.setString(1, size);
			ResultSet potRs = potPstmt.executeQuery();

			while (potRs.next()) {
				Map<String, String> pot = new HashMap<>();
				pot.put("id", potRs.getString("pot_id"));
				pot.put("name", potRs.getString("pot_name"));
				pot.put("price", potRs.getString("extra_price"));
				pot.put("image", potRs.getString("image_name"));
				potList.add(pot);
			}
			...
			potSql = "SELECT pot_id, pot_name, extra_price, image_name FROM pot WHERE level = '중상급자' AND size = ?";
			PreparedStatement potPstmt = conn.prepareStatement(potSql);
			potPstmt.setString(1, size);
			ResultSet potRs = potPstmt.executeQuery();
...
<script>
    ...
    function updateQuantity(change) {
	  ...
	  const potPrice = parseInt(potSelect.options[potSelect.selectedIndex].getAttribute("data-price")) || 0;
	  ...
	  const totalAmount = (productPrice + potPrice) * currentQuantity;
	  ...
	  // 화분 선택 값도 같이 업데이트
	  document.getElementById("selectedPotType_cart").value = potSelect.value;
	  document.getElementById("selectedPotType_buy").value = potSelect.value;
    ...
    <style>
      ...
      .nav-login {
       ...
       .footer {
		...
		.type td:first-child, .type td:last-child {
			width: 570px;
		}
...
<!--<%
	...
	Connection conNoob = null;
	PreparedStatement pstmtNoob = null;
	ResultSet rsNoob = null;
	String noob = "";
	int category = 0;

	try {
		String DB_URL_noob = "jdbc:mysql://localhost:3306/succu";
		String DB_ID_noob = "multi";
		String DB_PASSWORD_noob = "abcd";

		Class.forName("org.gjt.mm.mysql.Driver");
		conNoob = DriverManager.getConnection(DB_URL_noob, DB_ID_noob, DB_PASSWORD_noob);

		String sqlNoob = "SELECT noob, category FROM succu_experience WHERE id = ?";
		pstmtNoob = conNoob.prepareStatement(sqlNoob);
		pstmtNoob.setString(1, productId);
		rsNoob = pstmtNoob.executeQuery();

		if (rsNoob.next()) {
			noob = rsNoob.getString("noob");
			category = rsNoob.getInt("category");
		}
	} catch (Exception e) {
		out.println("에러: " + e.getMessage());
	} finally {
		if (rsNoob != null) try { rsNoob.close(); } catch (Exception e) {}
		if (pstmtNoob != null) try { pstmtNoob.close(); } catch (Exception e) {}
		if (conNoob != null) try { conNoob.close(); } catch (Exception e) {}
	}
%>-->
			...
			<div class="pay-path">
				<a href="sub1.jsp">다육 세트</a> &gt; 
				<% if ("O".equals(noob)) { %>
					<a href="sub1.jsp">초보</a>
				<% } else if ("X".equals(noob)) { %>
					<a href="Nonoob.jsp">중·상급자</a>
				<% } else { %>
					<span>분류없음</span>
				<% } %>
			</div>
           ...
           <div class="pay-price">
			...
			<div class="pot-wrapper">
			  <div class="pot-label">화분</div>
			  <div class="pot-options-container">

				<!-- 숨겨진 실제 값 전송용 -->
				<input type="hidden" name="potType" id="selectedPotId">
				<input type="hidden" name="potPrice" id="selectedPotPrice">

				<!-- 기존 select 숨김 처리 -->
				<select id="potSelect" name="potType" style="display: none;">
				  <% for (Map<String, String> pot : potList) {
					   String potId = pot.get("id");
					   String potName = pot.get("name");
					   String potPriceStr = pot.get("price");
					   String potImage = pot.get("image");
					   int potPrice = 0;
					   try { potPrice = Integer.parseInt(potPriceStr); } catch (Exception e) {}
				  %>
				  <option value="<%= potId %>" 
						  data-price="<%= potPriceStr %>" 
						  data-img="<%= (potImage != null ? potImage : "") %>">
					  <%= potName %>
				  </option>
				  <% } %>
				</select>
				...
				<div class="custom-dropdown" id="customDropdown" onclick="toggleDropdown()" >
					<div id="dropdown-selected" class="dropdown-selected" onclick="toggleDropdown()">
						<img id="selected-pot-img" class="dropdown-img" src="" alt="선택된 화분" style="display: none;">
						<span id="selected-pot-text">[필수] 선택</span>
					  </div>
					  <img src="images/arrow-big.png" class="dropdown-arrow" alt="열기" />

					  <ul id="dropdown-list" class="dropdown-list" style="display: none;">
						<% for (Map<String, String> pot : potList) {
							 String potId = pot.get("id");
							 String potName = pot.get("name");
							 String potPriceStr = pot.get("price");
							 String potImage = pot.get("image");
							 int potPrice = 0;
							 try { potPrice = Integer.parseInt(potPriceStr.replaceAll(",", "").trim()); } catch (Exception e) {}
						%>
						<li onclick="selectPot('<%= potId %>', '<%= potName %>', '<%= potPriceStr %>', '<%= potImage %>')">
						  <% if (potImage != null && !potImage.trim().equals("")) { %>
							<img src="images/<%= potImage %>" class="dropdown-img" />
						  <% } %>
						  <span><%= potName %> <%= (potPrice > 0 ? "(+" + String.format("%,d", potPrice) + "원)" : "") %></span>
						</li>
						<% } %>
					  </ul>
					</div>

					<input type="hidden" id="selectedPotId">
					<input type="hidden" id="selectedPotPrice">
					<input type="hidden" id="cartTotalAmount">
					<input type="hidden" id="buyTotalAmount">
            ...
            <div class="pay-buttons">
				<!-- 장바구니 버튼 -->
				<form method="POST" action="shopping_list.jsp" style="display: inline;" onsubmit="return syncCartPotSelection();">
					...
					<input type="hidden" name="pot_id" id="selectedPotType_cart" value=""> <!-- 선택된 pot ID 반영 -->
					<input type="hidden" name="potPrice" id="selectedPotPrice_cart" value=""> <!-- 선택된 pot 가격 반영 -->
					<button type="submit" class="pay-cart">장바구니</button>
				</form>

				<!-- 구매하기 버튼 -->
				<form method="POST" action="/succu/shopping_order_payment.jsp"
					  onsubmit="return syncBuyPotSelection() && validateBuyForm();">
					<input type="hidden" name="products" value="<%= product_id %>">
					<input type="hidden" name="quantities" value="<%= purchaseQuantity %>">
					<input type="hidden" name="totalAmount" value="<%= totalAmount %>">
					<input type="hidden" name="pot_id" id="selectedPotType_buy" value="">
					<input type="hidden" name="potPrice" id="selectedPotPrice_buy" value="">
					<input type="hidden" name="type" value="direct"> <%--  이거 꼭 추가 --%>
				...
				<script>
					function validateBuyForm() {
						...
						const amt = document.getElementById('buyTotalAmount').value;
	...
	<script>
	  ...
	  function toggleDropdown() {
		...
		list.style.display = dropdownVisible ? 'block' : 'none';
	  }
	  ...
	  function selectPot(id, name, priceStr, img) {
		// hidden 값 저장
		document.getElementById('selectedPotId').value = id;
		document.getElementById('selectedPotPrice').value = priceStr;

		// 이미지 처리
		const selectedImg = document.getElementById('selected-pot-img');
		if (img && img.trim() !== "") {
		  selectedImg.src = "images/" + img;
		...
		// 텍스트 처리
		const priceNum = parseInt(priceStr.replace(/,/g, '').trim()) || 0;
		const selectedText = document.getElementById('selected-pot-text');
		selectedText.innerText = name + (priceNum > 0 ? " (+" + priceNum.toLocaleString() + "원)" : "");
	  ...
	  function updateTotalAmount() {
		const productPrice = parseInt("<%= productPrice.replace(",", "") %>");
		const potPrice = parseInt(document.getElementById('selectedPotPrice').value || '0');
		const quantity = parseInt(document.getElementById('purchaseQuantity').value || '1');
		const total = (productPrice + potPrice) * quantity;

		document.getElementById("totalAmount").innerText = total.toLocaleString() + "원";
		document.getElementById("cartTotalAmount").value = total;
		document.getElementById("buyTotalAmount").value = total;
	  }
	</script>
...
<%
String toolPage = "tool.jsp"; // 기본 기본값

if ("O".equals(noob)) { // 초보자
    if (category == 1) {
        toolPage = "tool-sub1-noob-small.jsp"; // 초보자 소형
    } else if (category == 2) {
        toolPage = "tool-sub1-noob-medium.jsp"; // 초보자 중형
    } else if (category == 3) {
        toolPage = "tool-sub1-noob-large.jsp"; // 초보자 대형
    }
} else if ("X".equals(noob)) { // 중상급자
    if (category == 1) {
        toolPage = "tool-sub1-nonoob-small.jsp"; // 중상급자 소형
    } else if (category == 2) {
        toolPage = "tool-sub1-nonoob-medium.jsp"; // 중상급자 중형
    } else if (category == 3) {
        toolPage = "tool-sub1-nonoob-large.jsp"; // 중상급자 대형
    }
}
%>
	...
	<div class="set">
		...
		<table class="type">
			<tr>
				<td></td>
				<td onclick="showTool()">관리도구</td>
				<td onclick="showBook()">가이드북</td>
				<td></td>
			</tr>
		</table>
		<iframe id="contentSetTool" style="display: none; width: 1440px; height: 1650px; border: none;"></iframe>
		<iframe id="contentSetBook" style="display: none; width: 1440px; height: 550px; border: none;"></iframe>
	</div>
...
function showTool() {
    ...
    typeTabs[1].classList.add('active2'); // 관리도구 탭만 활성화

    document.getElementById('contentSetTool').style.display = 'block';
    document.getElementById('contentSetBook').style.display = 'none';
    document.getElementById('contentSetTool').src = '<%= toolPage %>';
}

// 가이드북 보여주기
function showBook() {
    typeTabs.forEach(tab => tab.classList.remove('active2'));
    typeTabs[2].classList.add('active2'); // 가이드북 탭만 활성화

    document.getElementById('contentSetTool').style.display = 'none';
    document.getElementById('contentSetBook').style.display = 'block';
    document.getElementById('contentSetBook').src = 'book.jsp';
}

// 처음 로딩될 때 자동으로 관리도구 클릭되게
window.onload = function() {
    document.querySelector('.raise td:nth-child(2)').click(); // 키우는 방법 '물'
    showTool(); // 구성품 '관리도구'
};
</script>
...
<table class="review">
    <tr>
        <td>리뷰</td>
        <td><a href="allReview.jsp?product_id=<%= productId %>">더보기</a></td>
...
<table class="inquiry">
    <tr>
        <td>문의</td>
        <td><a href="allInquiry.jsp?product_id=<%= product_id %>">더보기</a></td>
...
<div class="modal" id="inquiryModal">
    <div class="modal-content">
        ...
        <form id="inquiryForm" method="post" action="inquiry_process.jsp">
            ...
            <div class="explain">
                <p>작성한 문의는 <font>[마이페이지 > 문의]</font>에서 확인 및 수정이 가능합니다</p>
            </div>
...
<script>
function syncCartPotSelection() {
    // 선택된 화분 ID와 가격을 cart form에 반영
    const potId = document.getElementById("selectedPotId").value;
    const potPrice = document.getElementById("selectedPotPrice").value;

    document.getElementById("selectedPotType_cart").value = potId;
    document.getElementById("selectedPotPrice_cart").value = potPrice;

    return true; // 폼 전송 계속 진행
}
function syncBuyPotSelection() {
    const potId = document.getElementById("selectedPotId").value;
    const potPrice = document.getElementById("selectedPotPrice").value;
    ...
    document.getElementById("selectedPotType_buy").value = potId;
    document.getElementById("selectedPotPrice_buy").value = potPrice;
    return true;
}
