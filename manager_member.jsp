...
<%
    ...
    try {
        ...
        if (hasKeyword) {
            countSql += " WHERE user_id LIKE ? OR username LIKE ?";
        }
        ...
        // 2) 회원 조회 (검색 적용 + 페이징)
        String sql = "SELECT user_id, username, password, address, phone, mobile_phone, gender, email, birth_date " +
                     "FROM user";
        if (hasKeyword) {
...
<center>
<div class="title"><a href="manager_member.jsp">회원관리</a></div>

<div class="search">
  <form method="get" action="manager_member.jsp">
...
<table>
    <tr>
        ...
        <td style="width: 10%;">휴대전화번호</td>
        <td>성별</td>
        <td>이메일</td>
        <td>생년월일</td>
		<td style="width: 10%;">탈퇴</td>
    </tr>
...
%>
    <tr>
		...
		<td>
			<form method="post" action="manager_member_delete.jsp" onsubmit="return confirmDelete();">
...
%>
    <a href="manager_member.jsp?page=<%= currentPage - 1 %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>">&lt;</a>
<%
        }

        for (int i = 1; i <= totalPages; i++) {
%>
    <a href="manager_member.jsp?page=<%= i %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>"
