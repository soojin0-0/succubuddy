...
<%
    ...
    try {
		...
		String countSql = "SELECT COUNT(*) FROM order_detail";
       ...
       String orderSql = "SELECT o.order_date, o.user_id, od.status, od.detail_id, " +
                  "od.product_id, od.quantity, od.subtotal, od.pot_name, p.name " +
        ...
        while (rs.next()) {
            ...
            order.put("order_date", new java.text.SimpleDateFormat("yyyy.MM.dd").format(rs.getTimestamp("order_date")));
            order.put("user_id", rs.getString("user_id"));
            order.put("product_name", rs.getString("name"));
            order.put("pot_name", rs.getString("pot_name"));
            order.put("quantity", rs.getString("quantity"));
            order.put("subtotal", rs.getString("subtotal"));
            order.put("detail_id", rs.getString("detail_id"));
...
<script>
    function confirmDelete() {
        return confirm("정말로 이 회원을 탈퇴시키겠습니까?");
    }
</script>
</head>
<body>
...
<center>
<div class="title"><a href="manager_order.jsp">주문배송관리</a></div>

<div class="search">
  <form method="get" action="manager_order.jsp">
...
<% for (Map<String, String> order : orderList) { %>
    <tr>
        ...
        <td><%= order.get("pot_name") != null ? order.get("pot_name") : "-" %></td>
        <td><%= order.get("quantity") %></td>
        <td><%= String.format("%,d원", Integer.parseInt(order.get("subtotal"))) %></td>
        <td>
		  <form method="post" action="update_order_status.jsp">
			<input type="hidden" name="detail_id" value="<%= order.get("detail_id") %>">
			<select name="status" class="status-select">
				<option value="결제완료" <%= "결제완료".equals(order.get("status")) ? "selected" : "" %>>결제완료</option>
				<option value="배송준비중" <%= "배송준비중".equals(order.get("status")) ? "selected" : "" %>>배송준비중</option>
				<option value="배송중" <%= "배송중".equals(order.get("status")) ? "selected" : "" %>>배송중</option>
				<option value="배송완료" <%= "배송완료".equals(order.get("status")) ? "selected" : "" %>>배송완료</option>
				<option value="취소" <%= "취소".equals(order.get("status")) ? "selected" : "" %>>취소</option>
			</select>
			<button type="submit" style="margin-left:5px;">변경</button>
		  </form>
		</td>
...
%>
    <a href="manager_order.jsp?page=<%= currentPage - 1 %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>">&lt;</a>
<%
        }

        for (int i = 1; i <= totalPages; i++) {
%>
    <a href="manager_order.jsp?page=<%= i %><%= hasKeyword ? "&keyword=" + URLEncoder.encode(keyword, "euc-kr") : "" %>"
