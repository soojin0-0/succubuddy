...
<%
	...
	out.println("DEBUG - product_id: " + product_id + "<br>");
    String product_name = "";
    ...
    try {
        ...
        // 전체 리뷰 수 조회
        String countSql = "SELECT COUNT(*) FROM review WHERE product_id = ?";
        pstmt = conn.prepareStatement(countSql);
        pstmt.setString(1, product_id);
        ResultSet rsCount = pstmt.executeQuery();
        if (rsCount.next()) {
            totalReviews = rsCount.getInt(1);
		...
		String reviewSql = "SELECT review_id, user_id, review_text, review_score, DATE_FORMAT(review_ymd, '%Y.%m.%d') AS review_date " +
						   "FROM review WHERE product_id = ? ORDER BY review_ymd DESC LIMIT ?, ?";
...
%>
	<tr>
		<td>
			<div style="margin-bottom: 10px;"><%= reviewer %></div>
		...
		<td>
			<div style="max-width: 600px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
				<a href="all_review_detail.jsp?review_id=<%= reviewId %>">
					<%= text %>
				</a>
			</div>
		</td>
...
%>      <a class="disabled">&lt;</a>
<%      } else { %>
        <a href="all_review.jsp?page=<%= currentPage - 1 %>&product_id=<%= product_id %>">&lt;</a>
<%      }

        for (int i = 1; i <= totalPages; i++) {
            if (i == currentPage) {
%>      <a href="all_review.jsp?page=<%= i %>&product_id=<%= product_id %>" class="active"><b><%= i %></b></a>
<%          } else { %>
        <a href="all_review.jsp?page=<%= i %>&product_id=<%= product_id %>"><%= i %></a>
<%          }
        }
...
<%      }
        ...
        out.println("<tr><td colspan='3'>리뷰 불러오기 실패: " + e.getMessage() + "</td></tr>");
    } finally {
