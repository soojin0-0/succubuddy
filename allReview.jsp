	...
	<tr>
		...
		<td>
			<div style="max-width: 600px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
				<a href="allReviewDetail.jsp?review_id=<%= reviewId %>">
					<%= text %>
				</a>
			</div>
		</td>
...
%>      <a class="disabled">&lt;</a>
<%      } else { %>
        <a href="allReview.jsp?page=<%= currentPage - 1 %>&product_id=<%= product_id %>">&lt;</a>
<%      }

        for (int i = 1; i <= totalPages; i++) {
            if (i == currentPage) {
%>      <a href="allReview.jsp?page=<%= i %>&product_id=<%= product_id %>" class="active"><b><%= i %></b></a>
<%          } else { %>
        <a href="allReview.jsp?page=<%= i %>&product_id=<%= product_id %>"><%= i %></a>
<%          }
        }
