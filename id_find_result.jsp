...
<%
    ...
    if (username == null) username = "알 수 없음"; // 기본값 설정
    if (birth == null || birth.trim().isEmpty()) birth = "";
    if (phone == null || phone.trim().isEmpty()) phone = "";

    // 전화번호 형식 변환
    phone = phone.replaceAll("[^0-9]", ""); // 숫자만 남기기
    if (phone.length() == 10) {
        phone = phone.replaceAll("(\\d{3})(\\d{3})(\\d{4})", "$1-$2-$3"); // 010-XXX-XXXX
    } else if (phone.length() == 11) {
        phone = phone.replaceAll("(\\d{3})(\\d{4})(\\d{4})", "$1-$2-$3"); // 010-XXXX-XXXX
    }
    ...
    try {
        ...
        String sql = "SELECT user_id FROM user WHERE username = ? AND DATE_FORMAT(birth_date, '%y%m%d') = ? AND mobile_phone = ?";
    ...
    if (userId == null) {
        response.sendRedirect("id_find.jsp?error=1");
    } else {
%>
...
<body>
    ...
    <div class="container">
        <h2>아이디 찾기 결과</h2>
        <p class="result-text">입력하신 정보와 일치하는 아이디는 다음과 같습니다.</p>
        <p class="user-id"><%= username.length() > 0 ? username.substring(0, 1) + "*" : "알 수 없음" %>님의 아이디는 <span class="highlight"><%= userId %></span>입니다</p>
