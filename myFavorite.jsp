      ...
      .nav-login {
       ...
       .footer {
        ...
        .favorite-container {
			...
			gap: 40px; /* 쿠폰 간격 */
			max-width: 1100px; /* 한 줄에 3개 배치되도록 */
		}
...
<%
  try {
    ...
    // 관심상품 총 개수 구하기
    String countSql = "SELECT COUNT(*) FROM favorite WHERE user_id = ?";
    pstmt = con.prepareStatement(countSql);
    pstmt.setString(1, userId);
    rs = pstmt.executeQuery();
    if (rs.next()) totalCoupons = rs.getInt(1);
    rs.close(); pstmt.close();

    // 해당 페이지 상품 가져오기
    String sql = "SELECT p.product_id, p.name, p.price FROM favorite f JOIN product p ON f.product_id = p.product_id WHERE f.user_id = ? LIMIT ? OFFSET ?";
...
%>
    <div class="favorite">
      <a href="productDetail.jsp?product_id=<%= pid %>">
        <img src="images/<%= pid %>.jpg">
      </a>
      <div class="prd-name"><%= name %></div>
      <div class="prd-price"><%= String.format("%,d", price) %>원</div>
    </div>
<%
    }
...
<% if (!hasCoupons) { %>
  <p class="noFavorite">관심상품 목록이 없습니다.</p>
<% } %>

</div> <!-- .favorite-container 닫기 -->
...
<%
  ...
  if (currentPage > 1) {
%>
  <a href="myFavorite.jsp?page=<%= currentPage - 1 %>"><</a>
<% } %>
...
<%
  for (int i = 1; i <= totalPages; i++) {
    if (i == currentPage) {
%>
    <a href="myFavorite.jsp?page=<%= i %>" class="active"><%= i %></a>
<%
    } else {
%>
    <a href="myFavorite.jsp?page=<%= i %>"><%= i %></a>
<%
    }
  }
